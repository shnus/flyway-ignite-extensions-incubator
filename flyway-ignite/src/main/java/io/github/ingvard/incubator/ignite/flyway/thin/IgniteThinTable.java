...
public class IgniteThinTable extends Table<IgniteThinDatabase, IgniteThinSchema> {
     ...
     * Table lock string.
     */
    private final String tblLockStr = UUID.randomUUID().toString();
     ...
     */
    public IgniteThinTable(JdbcTemplate jdbcTemplate, IgniteThinDatabase db, IgniteThinSchema schema, String name) {
        super(jdbcTemplate, db, schema, name);
    }
    ...
    protected void doUnlock() throws SQLException {
        ...
        int competingLocksTaken = jdbcTemplate.queryForInt(
                "SELECT COUNT(*) FROM " + this + " WHERE " + database.quote("version") + " != '" + tblLockStr + "' AND "
        ...
        jdbcTemplate.executeStatement("DELETE FROM " + this + " WHERE " + database.quote("version") + " = '" + tblLockStr + "' AND "
                + database.quote("description") + " = 'flyway-lock'");
    }

    /**
     * Inserting locking row.
     */
    private boolean insertLockingRow() {
        ...
        Results results = jdbcTemplate.executeStatement(
                "INSERT INTO " + this + " VALUES (-100, '" + tblLockStr + "', 'flyway-lock', '', '', 0, '', now(), 0, TRUE)");
